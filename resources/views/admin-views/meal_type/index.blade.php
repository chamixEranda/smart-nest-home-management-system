...
<div class="container-fluid">
    ...
    <div class="page-header">
        <h1 class="page-header-title">
            <span class="page-header-icon">
                <img src="{{asset('assets/admin/images/types.png')}}" class="w--26" alt="">
            </span>
            <span>
                {{ translate('messages.add_new_type') }}
            </span>
        </h1>
    </div>
    <!-- End Page Header -->
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.meal-type.store') }}" method="POST" autocomplete="off">
                @csrf
                <div class="mb-3">
    ...
    <div class="card">
        <div class="card-header">
            <h5 class="card-title fw-semibold mb-4">{{ translate('messages.meal_type_list') }}</h5>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table id="account-table" class="table">
                    ...
                    <tbody>
                        @foreach ($lims_type_list as $key => $type)
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>{{ $type->name }}</td>
                            <td>
                                <div class="btn--container justify-content-center">
                                    <a class="btn action-btn btn--primary btn-outline-primary" href="javascript:"
                                        title="{{translate('messages.edit')}} {{translate('messages.type')}}"
                                        onclick="editMealType({{ $type->id }}, '{{ $type->name }}')"><i class="fas fa-pen"></i>
                                    </a>
                                    <a class="btn action-btn btn--danger btn-outline-danger" href="javascript:"
                                        onclick="form_alert('type-{{$type['id']}}','{{ translate('Want to delete this type ?') }}')"
                                        title="{{translate('messages.delete')}} {{translate('messages.type')}}"><i
                                            class="fas fa-trash"></i>
                                    </a>
                                    <form action="{{route('admin.meal-type.destroy',[$type['id']])}}"
                                        method="post" id="type-{{$type['id']}}">
                                        @csrf @method('delete')
                                    </form>
                                </div>
                            </td>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        {!! Form::open(['route' => ['admin.meal-type.update', 1], 'method' => 'put', 'id' =>
                        'edit-category-form']) !!}
