...
<div class="container-fluid">
    ...
    <div class="page-header">
        <h1 class="page-header-title">
            ...
            <span>
                {{ translate('messages.family_plans') }}
            </span>
        </h1>
    </div>
    <!-- End Page Header -->
    <div class="card">
        <div class="card-header">
            <div class="row">
                <div class="col">
                    <h5 class="card-title fw-semibold mb-4">{{ translate('messages.family_plan_list') }}</h5>
                </div>
            </div>
        </div>
        <div class="card-body">
            <table id="account-table" class="table">
                ...
                <tbody>
                    @foreach ($lims_family_plans as $key=> $plan)
                    <tr>
                        <td>{{ $key }}</td>
                        <td>{{ $plan->user->f_name.' '.$plan->user->l_name }}</td>
                        <td>
                            <img src="{{ asset('public/documents/plans/'.$plan->image) }}" class="card-img-top" alt="..."
                                onerror="this.src='{{ asset('assets/img/vacation.png') }}'" height="80" width="80" >
                        </td>
                        <td>{{ $plan->title }}</td>
